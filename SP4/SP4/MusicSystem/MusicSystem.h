...
public:
	...
	bool RegisterSound2D(std::string filename, std::string nametoberegister = "nil", bool withLoop = true ,bool withSoundEffectControl = true);
	...
	////Automatically create and add a CAudio class using the music filename inside soundlist,if nametoberegister is untouched, passed in audio filename will be use to be registerd.
	bool RegisterSound3D(std::string filename, std::string nametoberegister = "nil",irrklang::vec3df pos = irrklang::vec3df(0.f,0.f,0.f), bool withLoop = true ,bool withSoundEffectControl = true);

	//play a valid bgm track
	bool PlayBgmTrack(unsigned short trackindex);
	bool PauseBgmTrack(unsigned short trackindex,bool pause = true);
	bool PlayBgmTrack(std::string trackname);
	bool PauseBgmTrack(std::string trackname,bool pause = true);

	//play a valid sound track
	bool PlaySoundTrack(unsigned short trackindex);
	bool PauseSoundTrack(unsigned short trackindex,bool pause = true);
	bool PlaySoundTrack(std::string trackname);
